            ...
            if len(preds) < len(data):
                ...
                with torch.inference_mode():
                    for e, sample in enumerate(tqdm(data)):
                        ...
                        if CLIENT == 'cohere':
                            pred = cohere_api(cohere_client, input_text, system_text = '', model_name='command-r', max_tokens=16)
                        else: # client == 'openai'
                            pred = openai_api(openai_client, input_text, system_text = '', model_name = 'gpt-3.5-turbo', max_tokens=16)
